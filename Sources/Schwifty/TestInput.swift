...
/// Test string.
///
/// This is considered the ``Schwifty`` version of the Acid2 test. The compiler should support creating and assigning all the `var`/`let` operation, creating functions(`func foo() {...}`), and executing functions (`print()`)
///
/// **Note:**
...
var TestBlock: () -> Void = {
    ...
    let j = "jet"
    let k = "red. Kite"
    let l:UInt64 = 01234567891011 //This is done for the Swift Compiler on 32bit systems, casting will not be supported in Schwifty
    let m = 200e50
    let n = f
    let o = 0
